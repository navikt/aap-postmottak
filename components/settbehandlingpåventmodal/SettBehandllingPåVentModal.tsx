...
export const SettBehandllingPåVentModal = ({ isOpen, onClose, journalpostId, behandlingVersjon }: Props) => {
        ...
        {isOpen && (
          <form
            ...
            onSubmit={form.handleSubmit(async (data) => {
              setIsLoading(true);
              await settPåVent({
                referanse: Number(journalpostId),
                begrunnelse: data.begrunnelse,
                behandlingVersjon: behandlingVersjon,
                frist: formaterDatoForBackend(data.frist),
                grunn: data.grunn,
              });
              await revalidateFlyt(journalpostId);
              setIsLoading(false);
              onClose();
            })}
